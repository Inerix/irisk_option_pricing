...
for (i in 1:length(b)){
  V_l[i,1]=max(sum(S0*omega*exp((r-0.5*new_sigma^2)*T+new_sigma*b[i]))-strike,0)
}
...
for (k in 1:n){
  for (j in 2:J){
    V_l[j,k+1]=0.5*delta_t/delta_b^2*V_l[j-1,k]+(1-r*delta_t-delta_t/delta_b^2)*V_l[j,k]+0.5*delta_t/delta_b^2*V_l[j+1,k]
  }
}
